...
missing_percentages <- colSums(is.na(final_combined_data)) / nrow(final_combined_data) * 100
...
final_combined_data <- final_combined_data %>%
  arrange(date) %>%
  fill(Real_GDP, Disposable_Income, Corporate_Profits, Healthcare_Price_Index, .direction = "down") %>% 
  filter(!(is.na(unemployment_rate) & is.na(approval_rating))) %>% 
  group_by(president) %>% 
  mutate(
  ...
  ungroup() %>%
  fill(Real_GDP, Disposable_Income, Corporate_Profits, Healthcare_Price_Index, .direction = "up")

colSums(is.na(final_combined_data)) / nrow(final_combined_data) * 100
...
save(final_combined_data, file = "./data/complete_data.rda")
