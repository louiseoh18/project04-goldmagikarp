...
output$approval_plot <- renderPlot({
  pres_data() |>
    ggplot() +
    geom_line(aes(x = date, y = approval_rating, color = "Approval"), size = 1) +
    geom_line(aes(x = date, y = disapproval_rating, color = "Disapproval"), size = 1) +
    geom_line(aes(x = date, y = unsure_rating, color = "Unsure"), size = 1) +
    geom_vline(aes(xintercept = important_event_dates), size = 0.5) +
    geom_text(aes(x = important_event_dates, y = 103, label = important_events, hjust = -0.1)) +
...
helpText("Click to add more presidents. Use Backspace to remove them.")
