...
output$approval_plot <- renderPlot({
  ...
  # 2. Filter for ONLY the rows that have an event (remove NAs)
  ...
  ggplot(plot_data, aes(x = date)) +
    ...
    # Event Lines (Only use the filtered 'events_data')
    ...
    geom_text(data = events_data, 
              aes(x = date, y = 85, label = important_events), 
              angle = 90, vjust = -0.5, size = 4, color = "gray30") +
