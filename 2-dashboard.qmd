...
# TO-DO:
# Home page: review description and tutorial to make sure everything is clear
# Comparing presidents page: plot no longer rendering, needs to be debugged; also add instructions in sidebar
# Prediction page: provide model explanation --> if it's better for space, can also include on home page
# Reference page: maybe describe how the different data was used. We didn't end up using some of the data in the actual dashboard but we still analyzed it (which viewers won't see but can be made aware of)
```
...
**Welcome to Executive Economics!**\n
...
selectInput(
        ...
        selectize = FALSE,
        selected = "Joe Biden" # having Trump (2nd Term) pre-selected was not working
      )
```
...
output$comparison_approval_plot <- renderPlot({
  presi_data() |>
    ...
    geom_line(aes(y = approval_rating, color = "Approval"), linewidth = 1) +
    geom_line(aes(y = disapproval_rating, color = "Disapproval"), linewidth = 1) +
    geom_line(aes(y = unsure_rating, color = "Unsure"), linewidth = 1) +
    scale_color_manual(values = c("Approval" = "#61D04F", 
                                  "Disapproval" = "#DF536B", 
                                  "Unsure" = "#2297E6")) +
    scale_linetype_manual(values = c("solid", "dotted", "dashed", "dotdash", "longdash", "twodash",
                                     "12", "21", "13", "31", "14", "41", "1122", "2211", "F2")) +
    labs(y = "Rating (%)", x = "Time in Office (months)", 
           color = "Rating", linetype = "Presidents") +
    theme_minimal()
  })
```
...
output$comparison_approval_plot <- renderPlot({
  presi_data() |>
    ...
    scale_color_manual(values = c("Approval" = "#61D04F", 
                                  "Disapproval" = "#DF536B", 
                                  "Unsure" = "#2297E6")) +
    scale_linetype_manual(values = rep(c("solid", "dotted", "dashed", "dotdash", "longdash", "twodash"), 
                                       length.out = 15)) +
    labs(y = "Rating (%)", x = "Time in Office (months)", 
           color = "Rating", linetype = "Presidents") +
    theme_minimal()
...
observeEvent(input$reset, {
  ...
  output$forecastPlot <- renderPlot({
    ...
    ggplot(plot_df, aes(x = date, y = approval_rating, color = Type, linetype = Type, linewidth = Type)) +
      geom_line() +
